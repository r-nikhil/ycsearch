...
with open('data/yc-embedded.json', 'r') as embeddings_file:
    ...
    # sort by batch first (newest first), then by top_company status within each batch
    yc_companies.sort(key=lambda co: (co['batch'], 0 if co['top_company'] else 1), reverse=True)
